...
// Package externalnlb contains utilities for parsing Google Cloud Passthrough External Network Load Balancer logs.
package externalnlb // import "github.com/open-telemetry/opentelemetry-collector-contrib/extension/encoding/googlecloudlogentryencodingextension/internal/externalnlb"
...
var (
	ErrUnmarshalPayload  = errors.New("failed to unmarshal External NLB log payload")
...
func ParsePayloadIntoAttributes(payload []byte, attr pcommon.Map) error {
	...
	if err := shared.AddStrAsInt(gcpExternalNLBBytesReceived, log.BytesReceived, attr); err != nil {
		return fmt.Errorf("%w: %w", ErrBytesReceived, err)
	}

	if err := shared.AddStrAsInt(gcpExternalNLBBytesSent, log.BytesSent, attr); err != nil {
		return fmt.Errorf("%w: %w", ErrBytesSent, err)
	}

	if err := shared.AddStrAsInt(gcpExternalNLBPacketsReceived, log.PacketsReceived, attr); err != nil {
		return fmt.Errorf("%w: %w", ErrPacketsReceived, err)
	}

	if err := shared.AddStrAsInt(gcpExternalNLBPacketsSent, log.PacketsSent, attr); err != nil {
		return fmt.Errorf("%w: %w", ErrPacketsSent, err)
	}

	shared.PutStr(gcpExternalNLBRTT, log.RTT, attr)
